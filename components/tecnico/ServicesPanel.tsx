...
import { FREQUENT_SERVICES } from '../../data/mocks';
...
export const ServicesPanel = () => {
  return (
    <section className="w-full md:w-3/5 bg-gray-50 flex flex-col overflow-hidden" data-purpose="services-order">
      ...
      {/* Frequent Services Grid */}
      <div className="p-6 flex-1 overflow-y-auto border-b border-gray-200">
        <h2 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4">Servicios Frecuentes</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {FREQUENT_SERVICES.map(service => (
            <ServiceCard 
              key={service.id} 
              id={service.id} 
              name={service.name} 
      ...
      <div className="p-6 bg-white border-t border-gray-200 flex flex-col gap-4">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="bg-gray-900 text-white text-xs font-bold px-2.5 py-1 rounded-full">0 ITEMS</span>
        </div>
        <p className="text-sm text-gray-500 italic mb-4">No hay servicios seleccionados aún.</p>
        <button className="w-full bg-[#FFEC00] hover:bg-[#e6d400] text-gray-900 font-bold text-lg py-5 px-6 rounded-xl flex items-center justify-center gap-2 transition-colors shadow-sm mt-auto cursor-pointer">
