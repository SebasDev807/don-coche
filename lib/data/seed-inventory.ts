import { Product, ItemCategory } from '@prisma/client';
import { Decimal } from '@prisma/client/runtime/library';

/**
 * Función que genera un seed de 10 productos de inventario falsos
 * para simular la respuesta de la base de datos.
 * 
 * @returns {Product[]} Un arreglo de 10 objetos Product.
 */
export function getSeedProducts(): Product[] {
  const now = new Date();
  
  return [
    {
      id: '1',
      code: 'LUB-SN-10W30',
      name: 'Aceite 10W-30 Premium - Marca: Mobil 1 • Galón',
      category: ItemCategory.LUBRICANTES,
      stock: 45,
      unitCost: new Decimal(24.50),
      salePrice: new Decimal(38.00),
      profitPercentage: new Decimal(35.53),
      isActive: true,
      createdAt: now,
      updatedAt: now,
    },
    {
      id: '2',
      code: 'FIL-AS-0092',
      name: 'Filtro de Aceite Sintético - Marca: Bosch • Universal',
      category: ItemCategory.ACCESORIOS,
      stock: 5,
      unitCost: new Decimal(8.20),
      salePrice: new Decimal(15.50),
      profitPercentage: new Decimal(47.10),
      isActive: true,
      createdAt: now,
      updatedAt: now,
    },
    {
      id: '3',
      code: 'FRN-CER-BK9',
      name: 'Pastillas de Freno Cerámicas - Marca: Akebono • Juego Delantero',
      category: ItemCategory.SERVITECA,
      stock: 18,
      unitCost: new Decimal(45.00),
      salePrice: new Decimal(72.00),
      profitPercentage: new Decimal(37.50),
      isActive: true,
      createdAt: now,
      updatedAt: now,
    },
    {
      id: '4',
      code: 'LUB-MG-20W50',
      name: 'Aceite Mineral 20W-50 - Marca: Castrol • Cuarto',
      category: ItemCategory.LUBRICANTES,
      stock: 120,
      unitCost: new Decimal(6.50),
      salePrice: new Decimal(10.00),
      profitPercentage: new Decimal(35.00),
      isActive: true,
      createdAt: now,
      updatedAt: now,
    },
    {
      id: '5',
      code: 'LAV-SH-001',
      name: 'Shampoo con Cera - Marca: Meguiars • Galón',
      category: ItemCategory.LAVADERO,
      stock: 8,
      unitCost: new Decimal(35.00),
      salePrice: new Decimal(55.00),
      profitPercentage: new Decimal(36.36),
      isActive: true,
      createdAt: now,
      updatedAt: now,
    },
    {
      id: '6',
      code: 'ACC-AR-099',
      name: 'Aromatizante Pino - Marca: Little Trees • Unidad',
      category: ItemCategory.ACCESORIOS,
      stock: 200,
      unitCost: new Decimal(1.20),
      salePrice: new Decimal(3.50),
      profitPercentage: new Decimal(65.71),
      isActive: true,
      createdAt: now,
      updatedAt: now,
    },
    {
      id: '7',
      code: 'SRV-ALN-01',
      name: 'Pesas para Balanceo 5g - Marca: TR413 • Caja 100',
      category: ItemCategory.SERVITECA,
      stock: 3,
      unitCost: new Decimal(12.00),
      salePrice: new Decimal(25.00),
      profitPercentage: new Decimal(52.00),
      isActive: true,
      createdAt: now,
      updatedAt: now,
    },
    {
      id: '8',
      code: 'LUB-GR-002',
      name: 'Grasa Litio Multipropósito - Marca: Texaco • Libra',
      category: ItemCategory.LUBRICANTES,
      stock: 35,
      unitCost: new Decimal(4.50),
      salePrice: new Decimal(8.00),
      profitPercentage: new Decimal(43.75),
      isActive: true,
      createdAt: now,
      updatedAt: now,
    },
    {
      id: '9',
      code: 'LAV-DG-005',
      name: 'Desengrasante Motor - Marca: Simoniz • 5 Galones',
      category: ItemCategory.LAVADERO,
      stock: 12,
      unitCost: new Decimal(42.00),
      salePrice: new Decimal(70.00),
      profitPercentage: new Decimal(40.00),
      isActive: true,
      createdAt: now,
      updatedAt: now,
    },
    {
      id: '10',
      code: 'ACC-LM-010',
      name: 'Líquido Limpiaparabrisas - Marca: Qualitor • Galón',
      category: ItemCategory.ACCESORIOS,
      stock: 25,
      unitCost: new Decimal(5.80),
      salePrice: new Decimal(12.00),
      profitPercentage: new Decimal(51.66),
      isActive: true,
      createdAt: now,
      updatedAt: now,
    }
  ];
}
